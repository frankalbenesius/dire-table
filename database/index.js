...
const isValidTable = (key, snapshot) =>
  key && key.length === keyLength && snapshot.hasChild(key);
  ...
  database
    ...
    .then((snapshot) => {
      ...
      const myPlayerRef = database.ref(
        `tables/${tableKey}/players/${myPlayerKey}`
      );
      myPlayerRef.update({
        connected: true,
      });
      ...
      playersRef.transaction((players) => {
        if (players) {
          ...
          if (!myPlayer.name) {
            const currentNames = Object.values(players)
              .filter((p) => !!p.name)
              .map((p) => p.name);
