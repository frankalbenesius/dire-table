...
const removeTinyHoles = (shapes) => {
  shapes.forEach((shape) => {
    if (shape.paths.length > 1) {
      ...
      shape.paths = shape.paths.filter((s, i) => Math.abs(shapeAreas[i]) > minimumHoleArea);
    }
  });
  return shapes;
};
const mergeArea = (areas: Array<Area>, newArea: Area): Array<Area> => {
  const existingShapes = areas.map(area => new Shape(area, true, true).scaleUp(scaler));
  ...
  const mergedShapes = existingShape.union(newAreaShape).scaleDown(scaler).seperateShapes();
  const result = removeTinyHoles(mergedShapes).map(shape => shape.mapToLower());
  return result;
};
const removeArea = (areas: Array<Area>, newArea: Area): Array<Area> => {
  const existingShapes = areas.map(area => new Shape(area, true, true).scaleUp(scaler));
  const existingShape = existingShapes.reduce((acc, shape) => acc.join(shape), new Shape());
  const newAreaShape = new Shape(newArea, true, true).scaleUp(scaler);
  const mergedShapes = existingShape.difference(newAreaShape).scaleDown(scaler).seperateShapes();
  const result = removeTinyHoles(mergedShapes).map(shape => shape.mapToLower());
  return result;
};

const easement = 1 / scaler; // the amount of area put between areas
const toRect = (reduction: number) => (coordA: Coordinate, coordB: Coordinate = coordA): Area => {
...
const toAreaCursor = toRect(coordinateHalf - easement - magicNumberToReduceCursorSize);
